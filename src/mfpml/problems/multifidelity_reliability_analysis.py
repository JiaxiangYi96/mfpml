...
class mfRFunctions(Functions):

    def plot_limit_state(self, save_fig: bool = True,
                         fidelity: str = 'hf', factor=0.1, **kwargs) -> None:
        ...
        if num_dim == 2:
            ...
            for i in range(len(X1)):
                for j in range(len(X1)):
                    ...
                    if fidelity == 'hf':
                        ...
                        Y[i, j] = self.lf(x=xy, factor=factor)
            ...
            if save_fig is True:
                fig.savefig(self.__class__.__name__ + fidelity + ".png",
    ...
    def __call__(self, x: dict, **kwarg) -> dict:
        out = {}
        out["hf"] = None
        out["lf"] = None
        if x["hf"] is not None:
            out["hf"] = self.hf(x["hf"])
        if x["lf"] is not None:
            out["lf"] = self.lf(x["lf"], factor=kwarg["factor"])
        return out


class mfMultiModal(mfRFunctions):
    """_summary_
    ...
    def __init__(self, cost: list = [1, 0.1]) -> None:
        # calculate the cost ratio between high fidelity and low fidelity
        self.cr = cost[0]/cost[1]
    ...
    def lf(x: np.ndarray, factor: float) -> np.ndarray:
        ...
        # factor to control relation between high fidelity and low fidelity
        A = factor
...
class mfFourBranches(mfRFunctions):
    ...
    def lf(self, x, factor: float) -> np.ndarray:

        x1 = x[:, 0]
        x2 = x[:, 1]

        # get factor to control relation between high fidelity and low fidelity
        A = factor

        # The four branches
        g1 = 3 + (x1 - A * x2) ** 2 / 10 - (A * x1 + x2) / np.sqrt(2)
        g2 = 3 + (x1 - A * x2) ** 2 / 10 + (A * x1 + x2) / np.sqrt(2)
        g3 = (x1 - 0.9 * x2) + 7 / np.sqrt(2)
        g4 = -(x1 - 0.9 * x2) + 7 / np.sqrt(2)
