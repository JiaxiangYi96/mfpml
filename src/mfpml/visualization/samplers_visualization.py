...
class SFSamplerVisualization:
    ...
    def _plot_2D(self, samples: np.ndarray, save_figure: bool, filename: str):
        ...
        ax.scatter(samples[:, 0], samples[:, 1], c='#EE7733',
    ...
    def _plot_1D(self, samples: np.ndarray, save_figure: bool, filename: str):
        ...
        ax.scatter(samples[:, 0], np.zeros(samples.shape[0]),
                   c='#EE7733', marker='*', s=50, label='samples')
        plt.xlabel(r"$x_1$", fontsize=12)
...
class MFSamplerVisualization:
    ...
    def __init__(self, sampler: MFLatinHyperCube | MFSobolSequence):
        ...
        assert hasattr(
            self.sampler, 'data'), "Please generate the samples first."
...
if __name__ == '__main__':
    ...
    mf_sampler = MFLatinHyperCube(design_space=np.array(
        [[0, 1], [0, 1]]), num_fidelity=2, nested=True)
    # generate samples
