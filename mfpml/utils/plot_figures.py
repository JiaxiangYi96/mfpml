...
def plot_sf_sampling(
    samples: np.ndarray, responses: np.ndarray, function: Functions = None, save_figure: bool = False
) -> None:
    """
    ...
    if num_dim == 1:
        x_plot = np.linspace(start=function._input_domain[0, 0], stop=function._input_domain[0, 1], num=1000)
        ...
        num_plot = 200
        x1_plot = np.linspace(start=function._input_domain[0, 0], stop=function._input_domain[0, 1], num=num_plot)
        x2_plot = np.linspace(start=function._input_domain[1, 0], stop=function._input_domain[1, 1], num=num_plot)
        ...
        with plt.style.context(["ieee", "science"]):
            fig, ax = plt.subplots()
            plt.scatter(samples[:, 0], samples[:, 1], s=15, color="orangered", label="Samples")
...
def plot_mf_sampling(samples: dict, responses: dict, function: Functions = None, save_figure: bool = False) -> None:
    """
    Visualize the 1D case, set the y axis as Zero
    Parameters
    ----------
    ...
    if num_dim == 1:
        ...
        with plt.style.context(["ieee", "science"]):
            fig, ax = plt.subplots()
            ax.plot(samples["hf"][:, 0], responses["hf"][:, 0], "*", label="HF Samples")
            ax.plot(samples["lf"][:, 0], responses["lf"][:, 0], "o", label="LF Samples")
            ax.plot(x_plot, yh_plot, "-", label=f"High fidelity function")
            ax.plot(x_plot, yl_plot, "--", label=f"Low fidelity function")
            ax.legend()
            ax.set(xlabel=r"$x$")
            ax.set(ylabel=r"$y$")
            plt.xlim(left=function._input_domain[0, 0], right=function._input_domain[0, 1])
            if save_figure is True:
                fig.savefig(function.__class__.__name__, dpi=300)
            plt.show()
    elif num_dim == 2:
...
def plot_1d_model_prediction(
    data: dict, model: Any, function: Functions = None, name: str = "figure", save: bool = False
) -> None:
    """
    ...
    """
    x_plot = np.linspace(start=function.low_bound[0], stop=function.high_bound[0], num=1000)
    x_plot = x_plot.reshape((-1, 1))
