...
class Kriging:
    ...
    def train(self, X: np.ndarray, Y: np.ndarray) -> None:
        ...
        self.sigma2 = (
            np.dot((self.sample_Y - self.mu).T, self.gamma) / self.X.shape[0]
        ).item()
        self.logp = (
            -0.5 * self.X.shape[0] * np.log(self.sigma2)
    ...
    def _logLikelihood(self, params: np.ndarray) -> np.ndarray:
        ...
        for i in range(params.shape[0]):
            ...
            logp = -0.5 * self.X.shape[0] * np.log(sigma2) - np.sum(
                np.log(np.diag(L))
            )
            out[i] = logp.ravel()
        return -out
